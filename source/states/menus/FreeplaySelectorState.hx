...
class FreeplaySelectorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			for (touch in FlxG.touches.list) {
				if (controls.UI_UP_P || (touch.overlaps && touch.justPressed))
				{
					FlxG.sound.play(Paths.sound('scrollMenu'));
					changeItem(-1);
				}
				if (controls.BACK || (touch.overlaps && touch.justPressed))
				{
					FlxG.sound.play(Paths.sound('cancelMenu'));
					MusicBeatState.switchState(new MainMenuState());
				}

				if (controls.UI_DOWN_P || (touch.overlaps && touch.justPressed))
				{
					FlxG.sound.play(Paths.sound('scrollMenu'));
					changeItem(1);
				}

				if (controls.ACCEPT || (touch.overlaps && touch.justPressed))
				{
					selectedSomethin = true;
					FlxG.sound.play(Paths.sound('confirmMenu'));
